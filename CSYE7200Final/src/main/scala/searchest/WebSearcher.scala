...
class HtmlWriter(master: ActorRef) extends Actor {
  ...
  def receive: Receive = {
    case IndexMessage(url, content) =>
      ...
      val output1 = "C:\\Users\\XiaoCase\\Desktop\\CSYE7200Final\\output\\output1"
      val filename1 = url.toString.replaceAll("/", " ")
      ...
      val pw = new PrintWriter(new File(s"$output1\\$filename"))
